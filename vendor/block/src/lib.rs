...
// The Blocks runtime exports this symbol as storage for a class and callers
// only ever use its address. The original empty enum made the extern static
// uninhabited, which is future-incompatible in Rust. Match the concrete Apple
// ABI storage used by the maintained block-sys crate instead.
#[repr(C)]
struct Class {
    _priv: [*mut c_void; 32],
}
