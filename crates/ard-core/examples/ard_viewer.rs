...
impl Renderer {
    async fn new(window: Arc<Window>) -> Result<Self, String> {
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(include_str!("ard_viewer_mvs.wgsl").into()),
        });
    ...
    thread::spawn(move || {
        ...
        loop {
            let info = match client.next_event() {
                ...
                Ok(ArdClientEvent::MediaStream(_)) => continue,
                Err(error) => {
...
const VIEWER_USAGE: &str = "用法：ard_viewer [--quality low|medium|high|adaptive|full] [--frame-interval-ms 毫秒] 地址:5900 用户名";
...
fn main() {
    ...
    let password = match rpassword::prompt_password("ARD 密码：") {
        Ok(password) if !password.is_empty() => password.into_bytes(),
        Ok(_) => {
            eprintln!("密码不能为空");
            std::process::exit(2);
        }
        Err(error) => {
            eprintln!("无法从终端读取密码：{error}");
            std::process::exit(2);
        }
    };
...
mod tests {
    ...
    fn gpu_shader_is_valid_wgsl() {
        let module = naga::front::wgsl::parse_str(include_str!("ard_viewer_mvs.wgsl"))
